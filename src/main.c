...
int main() {
	...
	m->set_title(data, "游戏菜单");
	m->set_text(data, "1.开始游戏", "2.游戏记录", "3.游戏帮助", "4.清除存档", "5.设置", "0.退出游戏", NULL);
	m->set_text_data(data, "describe", "%s%s%s%s%s", "开始玩五子棋!好耶!", "查看所有记录下来的棋盘结果记录", "查看和游戏有关的帮助内容", "这会移除所有的记录，请慎重", "设置游戏内部的一些变量");

	m->set_title(help, "游戏帮助");
	m->set_type(help, "help");
	m->set_text(help, "1.按下0,q,Q,esc退出", "2.WSAD或者方向键控制方向，回车、空格下子", "3.@为黑棋,O为白棋,+为未下棋子", "4.可以参照界面的提示操作", "5.同时可以用方向键控制方向", "6.历史界面需要您的终端足够大以保持正常显示。按下r键可以进入单局回放，操作逻辑基本相同", "7.按照提示，键入Y删除，否则键入N或者其他按键", "8.倘若您使用当前目录作为数据的存储目录，则会提示是否删除直接退出程序防止再次创建文件夹", "9.同其他界面，使用WASD或方向键移动光标", "10.使用空格或者回车开启或者关闭光标所在选项", "11.棋盘大小可以使用+(或=)/-增加或者减少", "12.在绘图模式下，按下esc键保存退出，按下n键切换到下以张图", NULL);
