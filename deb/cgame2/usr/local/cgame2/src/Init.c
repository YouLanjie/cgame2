...
void Init() {
	...
	if(access(Config,0) == EOF) {       /* 创建Config文件 */
		...
		if(fscanf(fp, "%d%d%d%d", &config[0], &config[1], &config[2], &Max) == EOF) {
			perror("Config");
			printf("\033[?25h");
			exit(-1);
		}
	...
	else if (error != 1 && config[2] == 0) {  /* 更改数据存放地址为默认目录 */
	...
	if (!config[1]) {
		//拷贝内置数据到变量里
		strcpy(LANG[0], "\033[1;31m[main](remove Config): \033[0m");
		strcpy(LANG[1], "\033[1;31m[main](remove Save): \033[0m");
		strcpy(LANG[2], "是否直接退出游戏?否则将重新创建目录!(默认退出)Y/n\n");
		strcpy(LANG[3], "\033[?25h\033[2;25H\033[1;32m游戏设置\n\033[33mq键退出，退出保存\033[0m\n");
		strcpy(LANG[4], "\n由于使用的是当前目录，所以会删除数据文件夹\n");
		strcpy(LANG[5], "\033[1;33m请确清除存档，您将失去您的所有记录！（Y/n）\n");
		strcpy(LANG[6], "游戏时是否启用AI\033[4;25H( )\033[1;31m\033[4;29H|\033[0m使用英语\033[4;50H( )\n使用当前目录作为游戏目录\033[5;25H( )");
		// strcpy(LANG[7], "游戏帮助");
		strcpy(LANG[8], "将使用当前目录作为软件数据存放目录\n");
		// strcpy(LANG[9], "\033[7;4H按下0,q,Q退出\033[8;4HW S A D或者方向键控制上下左右，空格下子\033[9;4H@为黑棋,O为白棋,+为空白");
		// strcpy(LANG[10], "\033[8;%dH\033[1;33m1.开始游戏\033[8;%dH2.游戏记录\033[9;%dH3.游戏帮助\033[9;%dH4.清除存档");
		...
		strcpy(LANG[23], "\033[0;1;31m按下L查看上一局，按下N查看下一局,0退出\033[0m");
		strcpy(LANG[24], "\033[33m这已经是第一个记录了\033[0m\n");
		strcpy(LANG[25], "\033[33m这已经是最后一个记录了\033[0m\n");
	}
	else {
		//拷贝内置数据到变量里
		strcpy(LANG[0], "\033[1;31m[main](remove Config): \033[0m");
		strcpy(LANG[1], "\033[1;31m[main](remove Save): \033[0m");
		strcpy(LANG[2], "Do I exit the game directly? Otherwise the directory will be recreated! (Default exit)Y/n");
		strcpy(LANG[3], "\033[?25h\033[2;25H\033[1;32mGame settings\033[3;1H\033[33mq key to exit, exit save\033[0m");
		strcpy(LANG[4], "\nThe data folder will be deleted because the current directory is used");
		strcpy(LANG[5], "\033[1;33m Please make sure to clear the archive, you will lose all your records! (Y/n)");
		strcpy(LANG[6], "Whether AI is enabled during game play \033[4;26H( )\033[1;31m\033[4;29H|\033[0m Use English \033[4;48H( )\n Use current directory as game directory \033[5;26H( )");
		// strcpy(LANG[7], "Game Help");
		strcpy(LANG[8], "The current directory will be used as the software data storage directory");
		// strcpy(LANG[9], "\033[7;4H Press 0,q,Q to exit \033[8;4HW S A D or arrow keys to control up and down, space to play \033[9;4H@ for black, O for white, + for blank");
		...
		strcpy(LANG[23], "\033[0;1;31mPress L to view the previous game, press N to view the next game,0 to exit \033[0m");
