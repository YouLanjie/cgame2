...
void Init() {
	...
	if (!config[1]) {
		...
		strcpy(LANG[6], "游戏时是否启用AI\033[4;24H( )\033[1;31m\033[4;27H|\033[0m使用英语\033[4;48H( )\n使用当前目录作为游戏目录\033[5;24H( )");
		...
		strcpy(LANG[30], "是否直接退出游戏?否则将重新创建目录!(默认退出)Y/n\n");
		...
		strcpy(LANG[2], "\033[1;31m[init](Game Dir): access \033[0m");
		strcpy(LANG[3], "\033[4;1HWhether AI is enabled during game play \033[4;24H( )\033[1;31m\033[4;27H|\033[0m Use English \033[4;48H( )");
		strcpy(LANG[4], "\033[1;31m[init](mkdir) Failed to create folder \033[0m");
		strcpy(LANG[5], "\033[1;31m[init](Save): fopen\033[0m");
		strcpy(LANG[6], "\033[?25h\033[2;25H\033[1;32mGame settings\033[3;1H\033[33mq key to exit, exit save\033[0m");
		strcpy(LANG[7], "\033[1;31m[init](Config): fopen\033[0m");
		...
		strcpy(LANG[30], "Do I exit the game directly? Otherwise the directory will be recreated! (Default exit)Y/n");
		strcpy(LANG[31], "\033[1;31m[main](remove Save): \033[0m");
		strcpy(LANG[32], "\033[1;31m[main](remove Config): \033[0m");
	}
	...
	if (error == 1 || config[2] == 1) {  /* 更改数据存放地址 */
		...
		fp = fopen("./cgame2-data/config.txt","r");
		if (!fp) {
		...
		fp = fopen("/usr/local/cgame2/data/config.txt","r");
		if (!fp) {
